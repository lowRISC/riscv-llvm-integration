...
OperandMatchResultTy RISCVAsmParser::parseAtomicMemOp(OperandVector &Operands) {
  ...
  // parseImmediate followed by parseMemOpBaseReg, but these instructions do not
  // accept an immediate operand, and we do not want to add a "dummy" operand
  ...
  // will only print the register surrounded by parentheses (which GCC also uses
  // as its canonical representation for these operands).
  ...
  if (getLexer().isNot(AsmToken::LParen)) {
    ...
    SMLoc ImmEnd = getLoc();
    RVOp = RISCVOperand::createImm(MCConstantExpr::create(ImmVal, getContext()),
                                   ImmStart, ImmEnd, isRV64());
    FoundInteger = true;
  }

  if (getLexer().isNot(AsmToken::LParen)) {
    Error(getLoc(), FoundInteger ? "expected '(' after optional integer offset"
                                 : "expected '(' or optional integer offset");
    return MatchOperand_ParseFail;
  }
  getParser().Lex(); // Eat '('
  ...
  if (FoundInteger && !RVOp->isImmZero()) {
    Error(RVOp->getStartLoc(), "optional integer offset must be 0",
          SMRange(RVOp->getStartLoc(), RVOp->getEndLoc()));
    return MatchOperand_ParseFail;
  }
